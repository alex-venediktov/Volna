...
7. **У создания задачи бывают обязательные поля процесса.** Набор свой на каждом сервере
   ...
   `fieldReferenceName`. Значения задаются флагом `--field <ref>=<значение>` (повторяемый) или
   один раз переменной `TFS_CREATE_FIELDS` в `.env` — собирать запрос руками в обход CLI не надо.
...
человека, каждое изменение отдельно (этап `close`, флаг `--confirm` у CLI). Пакетная смена
статусов «заодно» запрещена процессом, а не только вежливостью.
