...
Проверить hooks без живой сессии: `node hooks/test-hooks.mjs .` из корня «Волны» — 74 проверки
на синтетическом `.volna`, включая поведение при битом `state.json`, удалённом каталоге,
чужом репозитории по соседству, профиле проекта без трекера и незаполненном профиле.
