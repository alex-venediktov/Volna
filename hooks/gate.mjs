#!/usr/bin/env node
/**
 * PreToolUse на Bash - ЕДИНСТВЕННОЕ блокирующее место «Волны»; из команд гейт разбирает свои
 * (commit и push) сам, потому что matcher отбирает инструмент, а не команду: ключа «условие по
 * команде» у hooks.json нет вовсе, и попытка его поставить кончалась предупреждением о чужом
 * ключе на каждом старте.
 *
