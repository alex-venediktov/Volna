...
export const DEFAULTS = {
  ...
  // Порядок значим: по нему сортируются колонка «Этапы» и шарды указателя (`lib/wiki-index.mjs`)
  stages: ["intake", "analyze", "spec", "plan", "implement", "advocate", "fixtures",
 ...
 */
export function unparsedLocators(text) {
  const out = [];
...
export function isIndexFile(name) {
 ...
 */
export function nodeOf(rel, topics = {}, depth = Infinity) {
...
export function nodeOfRel(rel, schema = DEFAULTS) {
 ...
 */
export function readRecords(root, deps = {}, schema = DEFAULTS) {
  const read = deps.readFile ?? ((p) => readFileSync(p, "utf8"));
  ...
  for (const path of list()) {
    ...
    for (const { heading, body } of bodies) {
      records.push({
        rel, path, section, node, fileTitle, heading, anchor: slug(heading), body,
