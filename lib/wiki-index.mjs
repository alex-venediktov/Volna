...
export function axisName(shardBy = INDEX_DEFAULTS.shard_by) {
  const names = { stage: "по этапам", topic: "по темам" };
  return shardBy.map((s) => names[s] ?? s).join(" и ");
}

/**
 * Служебные слова запроса: веса не дают. Отбор маршрута требует либо двух совпавших слов, либо
 * веса от трёх, а попадание в «предмет» даёт ровно три - значит одного «для» или «так», случайно
 * встреченного в предмете записи, хватало, чтобы увести запрос на чужой узел.
 *
 * Список короткий и только из служебных частей речи: предлоги, союзы, частицы, местоимения,
 * связки. Содержательное слово сюда не попадает - иначе запрос перестанет находить записи,
 * написанные этим словом. Слова пишутся **приведёнными** (`normText`): «ё» не различается,
 * и запись «всё» в списке была бы мёртвой.
 */
const STOP_WORDS = new Set([
  "без", "был", "была", "были", "было", "быть", "вот", "все", "где", "для", "его", "если",
  "есть", "еще", "или", "как", "кто", "либо", "меня", "мне", "над", "надо", "нас", "него",
  "них", "она", "они", "оно", "перед", "после", "при", "про", "себя", "так", "такой", "там", "тем",
  "того", "тоже", "только", "тот", "тому", "тут", "уже", "чем", "что", "чтоб", "чтобы", "эта",
  "эти", "этим", "это", "этого", "этой", "этому", "этот",
]);

/**
 * Аббревиатура: два-три знака в верхнем регистре, хотя бы один из них буква, как написал человек
 * (`ЛТ`, `ОС`, `ЦМ`, `У2`). Бизнес-акроним часто и есть ключ задачи, а порог «три буквы»
 * выбрасывал его целиком: запрос из одного такого слова давал «совпадений нет», а фраза
 * «для ЛТ сделать вот так» уводила на узел, совпавший служебным словом.
 *
 * Регистр смотрится до приведения: строчное «ос» - это предлог. Буква обязательна: число в два
 * знака («часть 12», «на 50») аббревиатурой не является и словом отбора быть не должно.
 */
function isAcronym(token) {
  return /^[\p{Lu}\p{N}]{2,3}$/u.test(token) && /\p{Lu}/u.test(token);
}

/**
 * Слова текста, годные для сопоставления: от трёх букв, регистр снят, служебные отброшены;
 * аббревиатуры в верхнем регистре проходят от двух. Один разбор на оба входа ядра -
 * «куда положить запись» и «какой указатель открыть под задачу».
 */
export function keywords(text) {
  const out = [];
  for (const token of String(text).match(/[\p{L}\p{N}]{2,}/gu) ?? []) {
    const word = normText(token);
    if (isAcronym(token)) { out.push(word); continue; }
    if (token.length < 3 || STOP_WORDS.has(word)) continue;
    out.push(word);
  }
  return [...new Set(out)];
}
 ...
 *
 * Аббревиатура из двух знаков ищется **целым словом**: подстрокой «ос» находится в «основе»,
 * «особенности» и «вопросе», то есть почти везде, и акроним перестал бы отбирать что бы то ни было.
 */
function wordHit(text, word) {
  if (word.length <= 2) return wholeWordHit(text, word);
  return normText(text).includes(word) || stemHit(text, word);
}

/**
 * Слово запроса в тексте без приведения основ: подстрокой, а для слова в два знака - целым словом.
 * Отличается от wordHit тем, что не ищет однокоренные: размещение записи и так считает вес по
 * четырём текстам сразу, и основа там даёт ложные узлы.
 */
function hasWord(text, word) {
  return word.length <= 2 ? wholeWordHit(text, word) : normText(text).includes(word);
}

/** Слово целиком среди слов текста. */
function wholeWordHit(text, word) {
  for (const token of normText(text).match(/[\p{L}\p{N}]+/gu) ?? []) {
...
export function matchDirs(records, text, schema = DEFAULTS) {
  ...
  for (const [dir, rows] of byDir) {
    ...
    // normText, а не toLowerCase: слова запроса приходят из keywords уже без «ё», и сравнение
    // с неприведённым текстом теряло бы запись, написанную через «ё»
    const segText = normText(segs.join(" "));
    const topicText = normText(segs.map((s) => topics[s] ?? "").join(" "));
    const subjText = normText(rows.map((r) => r.subject ?? "").join(" "));
    const headText = normText(rows.map((r) => r.heading ?? "").join(" "));
    let score = 0;
    const covered = [];
    for (const w of words) {
      // Сегмент пути весит больше предмета: он и есть заявленная тема узла, а предмет - частность
      const s = (hasWord(segText, w) ? 4 : 0) + (hasWord(topicText, w) ? 3 : 0)
        + (hasWord(subjText, w) ? 2 : 0) + (hasWord(headText, w) ? 1 : 0);
    ...
    const hitSegs = own.filter((s) => words.some((w) => hasWord(s, w) || hasWord(String(topics[s] ?? ""), w)));
    if (score) {
      scored.push({
